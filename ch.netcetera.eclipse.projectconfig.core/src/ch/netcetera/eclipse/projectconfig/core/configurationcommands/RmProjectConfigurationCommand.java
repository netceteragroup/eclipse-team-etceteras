...
/**
 * Project configuration command to delete a file (rm).
 */
public class RmProjectConfigurationCommand extends AbstractProjectConfigurationCommand {
   ...
   * @param log the log
   */
  public RmProjectConfigurationCommand(List<String> argumentList, ITextAccessor textAccessor, String pluginId, 
      ILog log) {
  ...
  IStatus executeOnProject(IProject project) {
    ...
    IPath pathToFile = project.getLocation().append(getArgumentList().get(1));
    File file = pathToFile.toFile();
    if (file != null && file.exists() && file.isFile()) {
      try {
        if (!file.delete()) {
          status = createStatus(IStatus.ERROR, getTextAccessor().getText("error.rm"));
        }
